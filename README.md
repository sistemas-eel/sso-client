...
[![Laravel Version](https://img.shields.io/badge/laravel-%5E8.0%7C%5E9.0%7C%5E10.0%7C%5E11.0%7C%5E12.0%7C%5E13.0-red.svg)](https://laravel.com)
...
Para validar sessões e detectar logout global, adicione o middleware `CheckSSOSession` ao grupo `web`. Em aplicações Laravel com sessão web, este middleware deve ser usado em produção para que eventos de logout global ou revogação encerrem a sessão local do usuário. Sem ele, o login OAuth continua funcionando, mas a aplicação cliente não reage ao logout global recebido pelo webhook. O local dessa configuração muda conforme a versão do Laravel.
...
```php
use SistemasEel\SSOClient\Laravel\Support\ClientPermission;
